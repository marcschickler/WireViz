...
def _entry_designator(entry: Any) -> str | None:
    ...
    if isinstance(entry, list) and entry and all(isinstance(x, str) for x in entry):
        # A list may contain multiple generated instances.  They must all be of
        # the same kind for a valid WireViz connection-set column.
        return entry[0]
    return None
...
def _is_connector_entry(entry: Any, connector_templates: set[str], separator: str) -> bool:
    ...
    for connection_set in data.get("connections", []):
        ...
        for current in connection_set[1:]:
            ...
            if (
                ...
                # therefore no cable node/BOM item. One marker per parallel
                # connection is generated later by the normal parser.
                expanded.append("--")
